...
class SingleCupGameDayContent extends SingleGameDayContent {
  final GameLeagueInfo gameLeagueInfo;

  const SingleCupGameDayContent({
    super.key,
  ...
  List<Widget> _buildGameGroup(GameGroup group) {
    if (group.groupIdentifier == seriesIdentifier) {
      return group.games.map((game) {
        if (game.seriesTitle != null) {
          return _LabeledSeriesGameRow(game, gameLeagueInfo);
        } else {
